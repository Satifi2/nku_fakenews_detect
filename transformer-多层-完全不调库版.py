...
def manual_dropout(x, dropout=0.5):
    """
    手动实现dropout效果。
    参数:
    - x: 输入张量。
    - dropout: dropout率，表示随机将张量中的一部分元素设为0的比例。
    返回:
    - 经过dropout处理的张量。
    """
    # 仅在训练模式下应用dropout，这里简化实现不考虑模式判断
    if dropout > 0:
        # 创建一个与x形状相同的mask，mask中的元素随机为0或1，1的概率为(1-dropout)
        mask = (torch.rand(x.shape,device=device) > dropout).float()
        # 应用mask，并重新缩放，以保持其总体的期望值不变
...
class SimpleTransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src):
        ...
        src = src + manual_dropout(src2,dropout)
        src = manual_layernorm(src)
        # print("shape of src", src.shape)

        src2 = manual_linear_forward(manual_dropout(manual_relu(manual_linear_forward(src,self.linear1)),dropout),self.linear2)
        src = src + manual_dropout(src2,dropout)
        src = manual_layernorm(src)
...
class TransformerModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, n_layers, n_heads, dropout=0.1):
        ...
        #自己实现的多层transformer encode
        self.transformer_encoder = MultiLayerTransformerEncoder(SimpleTransformerEncoderLayer(d_model=hidden_dim, n_heads=n_heads, dim_feedforward=hidden_dim, dropout=dropout),n_layers)
        self.output_fc = nn.Linear(hidden_dim, output_dim)

    def forward(self, x):
        # print("x.shape",x.shape)#torch.Size([64, 40, 300]
        # 调整输入维度
        x = manual_linear_forward(x,self.input_fc)  # [batch_size, seq_len, hidden_dim]
        ...
        # 输出层
        out = manual_linear_forward(out,self.output_fc)
