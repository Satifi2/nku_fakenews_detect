...
# 划分训练集和验证集
train_df, val_df = train_test_split(df, test_size=2000, random_state=42)

# 提取标题和标签
X_train = train_df['Title'].tolist()
y_train = train_df['label'].tolist()
X_validation = val_df['Title'].tolist()
y_validation = val_df['label'].tolist()
...
# 损失函数
loss_fn = nn.CrossEntropyLoss().to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
