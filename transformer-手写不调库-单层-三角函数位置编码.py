...
def position_encoding(pos, i, d_model):
    if i % 2 == 0:
...
class TransformerModel(nn.Module):
    ...
    def forward(self, x):
        ...
        position_enc = np.array([
            [position_encoding(pos, i, x.size(2)) for i in range(x.size(2))]
            for pos in range(seq_len)
        ])
        position_enc = torch.tensor(position_enc, dtype=torch.float32).to(x.device)
        x = x + position_enc
